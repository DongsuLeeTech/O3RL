...
parser.add_argument('--group', default='ABS-IQL-PEX')
parser.add_argument('--name', default='IQL-PEX')
...
def main(args, on_buffer, off_buffer=None):
    ...
    if args.offline_data:
        off_buffer.load(f"./buffers/{args.road}/{buffer_name}")

    # Initialize and load policy and Q_net
    offline_policy = ImplicitQLearning(args, num_inputs, num_actions, max_action, args.device)
    offline_policy.load(args.model_path)

    online_policy = ImplicitQLearning(args, num_inputs, num_actions, max_action, args.device)
    online_policy.pex_load(args.model_path)
    ...
    for en in range(args.num_evaluations):
        ...
        while horizon <= args.horizon:
            action = offline_policy.select_action(list(state.values()), deterministic=True)
    ...
    while ts <= args.epochs * args.horizon:
        ...
        while horizon < args.horizon:
            if args.render:
                env.render()
            off_action = offline_policy.select_action(list(state.values()), deterministic=False)
            action = online_policy.pex_select_action(list(state.values()), off_action, deterministic=False)
            episode_vel.append(list(state.values())[0])
            next_state, reward, done, _ = env.step({list(state.keys())[0]: action})
            tot_reward += list(reward.values())[0]
            on_buffer.add(list(state.values())[0], action,
                              list(next_state.values())[0], list(reward.values())[0], done['__all__'] * 1)

            if ts >= args.batch * 10:
                online_policy.train(on_buffer, off_buffer)
    ...
    online_policy.save(f'./results/{args.log_dir}/{args.log_dir}')
...
if __name__=="__main__":
    ...
    for j in seed_list:
        ...
        if args.offline_data:
            offline_replay_buffer = ReplayBuffer(state_dim, action_dim, args.device, buffer_size)
        else:
            offline_replay_buffer = None
        online_replay_buffer = ReplayBuffer(state_dim, action_dim, args.device, buffer_size)

        from datetime import datetime
        date = datetime.today().strftime("[%Y|%m|%d|%H:%M:%S]")
        args.log_dir = f"{date}-{args.road}-{buffer_name}-{args.seed}-PEX"
        os.mkdir(f'./results/{args.log_dir}')

        print('-----------------------------------------------------')
        main(args, online_replay_buffer, offline_replay_buffer)
        wandb.finish()
        args.name = 'IQL-PEX'
