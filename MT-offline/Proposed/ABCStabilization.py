...
class ABCSTAB(object):
    ...
    def stabilization(self, replay_buffer):
        ...
        v = self.v(state)
        v_loss = F.mse_loss(qft, v)
        self.v_optimizer.zero_grad(set_to_none=True)
        v_loss.backward()
        self.v_optimizer.step()

        target = reward + (1 - not_done) * self.discount * qft.detach()
        policy_out = self.actor(state)
        entropy = policy_out.log_prob(policy_out.sample())
        target = target +  self.discount * self.temperature * entropy.detach()
    ...
    def fine_tuning(self, replay_buffer):
        batch = replay_buffer.sample(self.batch)
        batch = [b.to(self.device) for b in batch]
        (state, action, next_state, reward, not_done) = batch
        ...
        if isinstance(policy_out, torch.distributions.Distribution):
            loss = -policy_out.log_prob(policy_out.rsample())
        else:
