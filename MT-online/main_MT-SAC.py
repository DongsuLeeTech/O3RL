...
parser.add_argument('--actor_lr', type=float, default=3e-04,)
parser.add_argument('--critic_lr', type=float, default=3e-04,)

# SAC parameter
parser.add_argument('--alpha', type=float, default=0.01,)
parser.add_argument('--alpha_lr', type=float, default=1e-04,)
parser.add_argument('--policy-type', type=str, default='Stochastic')
parser.add_argument('--target-update-interval', type=int, default=2)

parser.add_argument('--project', default='ABS')
parser.add_argument('--group', default='ABS-MT-SAC')
parser.add_argument('--name', default='MT-SAC-online')
...
def main(args):
    ...
    # Replay buffer
    replay_buffer = ReplayBuffer(num_inputs, num_actions, args.device)

    # agent
    policy = SAC(args, num_inputs, num_actions, max_action)
    ...
    for ep in range(1, args.epochs + 1):
        for env, road in zip(env_list, ['MA_4BL', 'MA_5LC', 'UnifiedRing']):
            ...
            while horizon < args.horizon:
                ...
                action = policy.select_action(list(state.values()), evaluate=False)
                episode_vel.append(list(state.values())[0])
                next_state, reward, done, _ = env.step({list(state.keys())[0]: action[0]})
...
if __name__=="__main__":
    ...
    for j in seed_list:
        ...
        args.name = f"{args.name}-Seed{args.seed}-{args.road}-{str(uuid.uuid4())[:8]}"
        config = vars(args)
        wandb_init(config)
