...
# fed_avg
parser.add_argument('--global_eval_freq', type=int, default=5,)

parser.add_argument('--project', default='ABS')
parser.add_argument('--group', default='ABS-MT-IL-ft-forgetting')
parser.add_argument('--name', default='MT-IL-ft-forgetting')
...
def main(args, replay_buffer):
    ...
    policy = DDPGBC(args, num_inputs, num_actions, max_action, args.device)
    ...
    for env, road in zip(env_list, ['MA_4BL', 'MA_5LC', 'UnifiedRing']):
        ...
        wandb.log(
            ...
            step=0)
        wandb.log(
            {f"{road}-vanilla_reward-global": eval_reward,
             f"{road}-correction_reward-global": correction_reward,
             f"{road}-timesteps-global": eval_timestep},
            step=int(0))
    ...
    for ep in range(1, args.epochs + 1):
        for env, road in zip([env_list[-1]], ['UnifiedRing']):
            if road == 'MA_4BL':
                warmup_ts = 900
            elif road == 'MA_5LC':
                warmup_ts = 125
            ...
            wandb.log(
                {f"{road}-vanilla_reward": eval_reward, f"{road}-correction_reward": correction_reward,
                 f"{road}-timesteps": eval_timestep},
                step=int(ep))

        tot_ep += 1.

        # global evaluation
        if tot_ep % args.global_eval_freq == 0 or tot_ep < args.global_eval_freq:
            for env, road in zip(env_list, ['MA_4BL', 'MA_5LC', 'UnifiedRing']):
                if road == 'MA_4BL':
                    warmup_ts = 900
                elif road == 'MA_5LC':
