...
parser.add_argument('--group', default='ABS-MT-IL-FedAvg-fine-tuning')
parser.add_argument('--name', default='MT-IL-FedAvg-finetune')
...
def main(args):
    ...
    global_policy = DDPGBC(args, num_inputs, num_actions, max_action, args.device)
    global_policy.load(args.model_path)

    policy1 = deepcopy(global_policy)
    policy2 = deepcopy(global_policy)
    policy3 = deepcopy(global_policy)
    ...
    for ep in range(1, args.epochs + 1):
        for idx, env in enumerate(env_list):
            ...
            wandb.log(
                {f"{road_list[idx]}-vanilla_reward": eval_reward,
                 f"{road_list[idx]}-correction_reward": correction_reward,
                 f"{road_list[idx]}-timesteps": eval_timestep},
                step=int(ep))
...
def global_policy_update(p_list, global_policy):
    # global_q = p_list[0].critic.parameters()
    # global_q_target = p_list[0].critic_target.parameters()
    global_actor = p_list[0].actor.parameters()
    # global_actor_target = p_list[0].actor_target.parameters()

    for i in range(1, len(p_list)):
        # for global_param, param in zip(global_q, p_list[i].critic.parameters()):
        #     global_param.data.add_(param.data, alpha=1)
        # for global_param, param in zip(global_q_target, p_list[i].critic_target.parameters()):
        #     global_param.data.add_(param.data, alpha=1)
        for global_param, param in zip(global_actor, p_list[i].actor.parameters()):
            global_param.data.add_(param.data, alpha=1)
        # for global_param, param in zip(global_actor_target, p_list[i].actor_target.parameters()):
        #     global_param.data.add_(param.data, alpha=1)

    # for global_param, param in zip(global_q, global_policy.critic.parameters()):
        # global_param.data /= len(p_list)
        # param.data = global_param
    # for global_param, param in zip(global_q_target, global_policy.critic_target.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    for global_param, param in zip(global_actor, global_policy.actor.parameters()):
        global_param.data /= len(p_list)
        param.data = global_param
    # for global_param, param in zip(global_actor_target, global_policy.actor_target.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
...
if __name__=="__main__":
    ...
    for i in env_list:
        ...
        for j in seed_list:
            args.seed = j
            args.model_path = './model/MT-IL/' + i + f'/Seed{j}/'
            ...
            main(args)
            wandb.finish()
            args.name = 'MT-IL-FedAvg-finetune'
