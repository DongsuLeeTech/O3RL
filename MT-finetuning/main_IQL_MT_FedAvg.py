...
parser.add_argument('--group', default='ABS-MT-IQL-FedAvg-fine-tuning')
parser.add_argument('--name', default='MT-IQL-FedAvg-finetune')
...
def main(args):
    ...
    global_policy = ImplicitQLearning(args, num_inputs, num_actions, max_action, args.device)
...
def global_policy_update(p_list, global_policy):
    # global_q = p_list[0].q.parameters()
    # global_q_target = p_list[0].q_target.parameters()
    # global_v = p_list[0].v.parameters()
    global_actor = p_list[0].actor.parameters()

    for i in range(1, len(p_list)):
        # for global_param, param in zip(global_q, p_list[i].q.parameters()):
        #     global_param.data.add_(param.data, alpha=1)
        # for global_param, param in zip(global_q_target, p_list[i].q_target.parameters()):
        #     global_param.data.add_(param.data, alpha=1)
        # for global_param, param in zip(global_v, p_list[i].v.parameters()):
        #     global_param.data.add_(param.data, alpha=1)
        for global_param, param in zip(global_actor, p_list[i].actor.parameters()):
            global_param.data.add_(param.data, alpha=1)

    # for global_param, param in zip(global_q, global_policy.q.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    # for global_param, param in zip(global_q_target, global_policy.q_target.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    # for global_param, param in zip(global_v, global_policy.v.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
...
if __name__=="__main__":
    ...
    for i in env_list:
        ...
        for j in seed_list:
            args.seed = j
            args.model_path = './model/MT-IQL/' + i + f'/Seed{j}/'
            ...
            main(args)
            wandb.finish()
            args.name = 'MT-IQL-FedAvg-finetune'
