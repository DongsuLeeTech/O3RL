...
# fed_avg
parser.add_argument('--global_eval_freq', type=int, default=2,)

parser.add_argument('--project', default='ABS')
parser.add_argument('--group', default='ABS-MT-AWAC-FedAvg-fine-tuning')
parser.add_argument('--name', default='MT-AWAC-FedAvg-finetune')
...
def main(args):
    ...
    buffer_size = len(np.load(f"./buffers/{args.road}/{args.dataset}/reward.npy"))
    replay_buffer1 = ReplayBuffer(state_dim, action_dim, args.device, buffer_size, args.off_data)
    replay_buffer2 = ReplayBuffer(state_dim, action_dim, args.device, buffer_size, args.off_data)
    replay_buffer3 = ReplayBuffer(state_dim, action_dim, args.device, buffer_size, args.off_data)
    replay_buffer_list = [replay_buffer1, replay_buffer2, replay_buffer3]

    # Initialize and load policy and Q_net
    global_policy = AWAC(args, num_inputs, num_actions, max_action)
    global_policy.fed_load(args.model_path)

    policy1 = copy.deepcopy(global_policy)
    policy2 = copy.deepcopy(global_policy)
    policy3 = copy.deepcopy(global_policy)
    policy_list = [policy1, policy2, policy3]

    road_list = ['MA_4BL', 'MA_5LC', 'UnifiedRing']
    # Replay buffer
    ...
    for idx, env in enumerate(env_list):
        print(env)
        if road_list[idx] == 'MA_4BL':
            warmup_ts = 900
        elif road_list[idx] == 'MA_5LC':
            warmup_ts = 125
        elif road_list[idx] == 'UnifiedRing':
            warmup_ts = 90
        ...
        for _ in range(args.num_evaluations):
            ...
            while ts <= args.max_ts:
                if args.render:
                    env.render()
                action = policy_list[idx].select_action(list(state.values()))
                ...
                if args.warmup_sample:
                    replay_buffer_list[idx].add(list(state.values())[0], action,
        ...
        print('----------------------------------------------------------------------------------------')
        print('# itr: {} # avg.reward: {} # cor.reward: {}'.format(road_list[idx], eval_reward, correction_reward))
        print('# velocity list: {} over {} evaluations'.format(velocity, args.num_evaluations))
        print('# average episode len: {}'.format(eval_timestep))
        print('----------------------------------------------------------------------------------------')

        wandb.log(
            {f"{road_list[idx]}-vanilla_reward": eval_reward, f"{road_list[idx]}-correction_reward": correction_reward,
             f"{road_list[idx]}-timesteps": eval_timestep},
            step=0)

        wandb.log(
            {f"{road_list[idx]}-vanilla_reward-global": eval_reward, f"{road_list[idx]}-correction_reward-global": correction_reward,
             f"{road_list[idx]}-timesteps-global": eval_timestep},
            step=0)
    ...
    for ep in range(1, args.epochs+1):
        for idx, env in enumerate(env_list):
            ...
            while horizon < args.horizon:
                if args.render:
                    env.render()
                action = policy_list[idx].select_action(list(state.values()), deterministic=False)
                episode_vel.append(list(state.values())[0])
                next_state, reward, done, _ = env.step({list(state.keys())[0]: action})
                tot_reward += list(reward.values())[0]
                replay_buffer_list[idx].add(list(state.values())[0], action,
                ...
                policy_list[idx].train(replay_buffer_list[idx])
        ...
        # global update
        policy_list, global_policy = global_policy_update(policy_list, global_policy)

        tot_ep += 1.

        # global evaluation
        if tot_ep % args.global_eval_freq == 0:
            for idx, env in enumerate(env_list):
                if road_list[idx] == 'MA_4BL':
                    warmup_ts = 900
                elif road_list[idx] == 'MA_5LC':
                ...
                while horizon < args.horizon:
                    ...
                    tot_reward += list(reward.values())[0]
                    replay_buffer_list[idx].add(list(state.values())[0], action,
                                                list(next_state.values())[0], list(reward.values())[0],
                                                done['__all__'] * 1)
                ...
                wandb.log(
                    {f"{road_list[idx]}-vanilla_reward-global": eval_reward,
                     f"{road_list[idx]}-correction_reward-global": correction_reward,
                     f"{road_list[idx]}-timesteps-global": eval_timestep},
                    step=int(tot_ep))

    global_policy.save(f'./results/{args.log_dir}/{args.log_dir}')

def global_policy_update(p_list, global_policy):
    # global_q1 = p_list[0].q1.parameters()
    # global_q1_target = p_list[0].q1_target.parameters()
    # global_q2 = p_list[0].q2.parameters()
    # global_q2_target = p_list[0].q2_target.parameters()
    global_actor = p_list[0].actor.parameters()

    for i in range(1, len(p_list)):
        # for global_param, param in zip(global_q1, p_list[i].q1.parameters()):
        #     global_param.data.add_(param.data, alpha = 1)
        # for global_param, param in zip(global_q1_target, p_list[i].q1_target.parameters()):
        #     global_param.data.add_(param.data, alpha = 1)
        # for global_param, param in zip(global_q2, p_list[i].q2.parameters()):
        #     global_param.data.add_(param.data, alpha = 1)
        # for global_param, param in zip(global_q2_target, p_list[i].q2_target.parameters()):
        #     global_param.data.add_(param.data, alpha = 1)
        for global_param, param in zip(global_actor, p_list[i].actor.parameters()):
            global_param.data.add_(param.data, alpha = 1)

    # for global_param, param in zip(global_q1, global_policy.q1.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    # for global_param, param in zip(global_q1_target, global_policy.q1_target.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    # for global_param, param in zip(global_q2, global_policy.q2.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    # for global_param, param in zip(global_q2_target, global_policy.q2_target.parameters()):
    #     global_param.data /= len(p_list)
    #     param.data = global_param
    for global_param, param in zip(global_actor, global_policy.actor.parameters()):
        global_param.data /= len(p_list)
        param.data = global_param

    return [global_policy, global_policy, global_policy], global_policy
...
if __name__=="__main__":
    seed_list = [1, 2, 3]
    env_list = ['final-medium', 'humanlike', 'medium', 'medium-random', 'final-random']
    args.road = 'mixed_env'
    for i in env_list:
        args.dataset = i
        for j in seed_list:
            args.seed = j
            args.model_path = './model/MT-AWAC/' + i + f'/Seed{j}/'
            ...
            main(args)
            wandb.finish()
            args.name = 'MT-AWAC-FedAvg-finetune'
